...
from feature_distill_four_lowc import F_D
...
TRAIN_LEARNING_RATE_D=1e-4#4
TRAIN_LEARNING_RATE_D_ddcpp=1e-4

TRAIN_GANLOSS='BCE'
TRAIN_LAMBADA_ADV_FIRST = 0.03
...
TRAIN_LEARNING_RATE_D_Variance = 0

SHOW = True

op_c_list = get_op_c(LEVEL = LEVEL)

model_trans = 'cvt_official' #cvt#vit#cvt_official
CONCAT_1_2 = False
IF_SAM = False
IF_SC_dis = False
...
def train(hyp, opt, device, tb_writer=None, MR=True):
    ...
    dis_model = get_model(model=model_trans)
    dis_model_front = get_model(model='cvt_official_18')
    generate_F_B_model = F_D(input_channel=18)
    generate_F_G_model = get_F_G()
    dis_model.train()
    dis_model_front.train()
    generate_F_B_model.train()
    generate_F_G_model.train()
    ...
    if cuda and rank == -1 and torch.cuda.device_count() > 1:
        ...
        dis_model_front = torch.nn.DataParallel(dis_model_front)
        generate_F_B_model = torch.nn.DataParallel(generate_F_B_model)
        generate_F_G_model = torch.nn.DataParallel(generate_F_G_model)

    # SyncBatchNorm
    if opt.sync_bn and cuda and rank != -1:
        model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model).to(device,non_blocking=True)
        dis_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(dis_model).to(device,non_blocking=True)
        dis_model_front = torch.nn.SyncBatchNorm.convert_sync_batchnorm(dis_model_front).to(device, non_blocking=True)
        generate_F_B_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(generate_F_B_model).to(device,non_blocking=True)
        generate_F_G_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(generate_F_G_model).to(device,non_blocking=True)
    ...
    # batch_size_trg = int(42782 / 2975 * batch_size / len_device) * len_device
...
###########################################################################################################################
    ...
    if cuda and rank != -1:
        ...
        dis_model = DDP(dis_model, device_ids=[opt.local_rank], output_device=opt.local_rank, find_unused_parameters=any(isinstance(layer, nn.MultiheadAttention) for layer in model.modules()))
        dis_model_front = DDP(dis_model_front, device_ids=[opt.local_rank], output_device=opt.local_rank,
                        find_unused_parameters=any(
                            isinstance(layer, nn.MultiheadAttention) for layer in model.modules()))
        generate_F_B_model = DDP(generate_F_B_model, device_ids=[opt.local_rank], output_device=opt.local_rank, find_unused_parameters=any(isinstance(layer, nn.MultiheadAttention) for layer in model.modules()))
        generate_F_G_model = DDP(generate_F_G_model, device_ids=[opt.local_rank], output_device=opt.local_rank, find_unused_parameters=any(isinstance(layer, nn.MultiheadAttention) for layer in model.modules()))
    ...
    optimizer_d_first = optim.Adam(dis_model.parameters(), lr=TRAIN_LEARNING_RATE_D_ddcpp*5,
                                 betas=(0.9, 0.99))
    optimizer_d_first_front = optim.Adam(dis_model_front.parameters(), lr=TRAIN_LEARNING_RATE_D_ddcpp*5,
                                   betas=(0.9, 0.99))
    optimizer_cycle_B = optim.Adam(generate_F_B_model.parameters(), lr=TRAIN_LEARNING_RATE_D_ddcpp,
                                 betas=(0.9, 0.99))
    optimizer_cycle_G = optim.Adam(generate_F_G_model.parameters(), lr=TRAIN_LEARNING_RATE_D_ddcpp,
                                 betas=(0.9, 0.99))
    if IF_SC_dis:
        lf_d_first = one_cycle(1, 0.2, epochs)  # cosine 1->hyp['lrf']#余弦退火衰减
        scheduler_d_first = lr_scheduler.LambdaLR(optimizer_d_first, lr_lambda=lf_d_first )
        scheduler_d_first.last_epoch = -1
...
#######################################################################
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        optimizer_d_first_front.zero_grad()
        optimizer_cycle_B.zero_grad()
        optimizer_cycle_G.zero_grad()
###########################################################下面就是按照iter计数

        # for i, (imgs, targets, paths, _),_, batch in pbar :  # batch -------------------------------------------------------------
        for i, (src,dis_src,dis_trg) in pbar:#源域和目标域
            dis_model = dis_model.to(device)#dis_model.train()
            dis_model_front = dis_model_front.to(device)
            generate_F_B_model = generate_F_B_model.to(device)
            generate_F_G_model = generate_F_G_model.to(device)
            ...
            if pattern == 'train':
                src_show_i = find_pict(pict_name='data/cityscapes/images/train_full/aachen/aachen_000031_000019_leftImg8bit.png',
                                       paths=paths)
                trg_show_i = find_pict(pict_name='data/Caltech/images/train_small/set00/set00_V000_242.png',  # _small
                                       paths=paths_trg)
            if pattern == 'debug':
                src_show_i = find_pict(pict_name='data/cityscapes/images/train_debug/aachen/aachen_000031_000019_leftImg8bit.png',
                                       paths=paths)#_full
                trg_show_i = find_pict(pict_name='data/Caltech/images/train_debug/set00/set00_V000_242.png',#_small
                                       paths=paths_trg)
            ...
            ################################# 1、yoloV5正常训练源域图片#############################
            dis_model = deal_grad(dis_model, key=False)
            dis_model_front = deal_grad(dis_model_front, key=False)
            generate_F_B_model = deal_grad(generate_F_B_model, key=False)
            generate_F_G_model = deal_grad(generate_F_G_model, key=False)
            ...
            if Cheat: #让目标域的图片被认为来自源域
                with amp.autocast(enabled=cuda):
                    feature_src_B = generate_F_B_model(feature_src)
                    feature_trg_B = generate_F_B_model(feature_trg)
                    d_out_first = dis_model(feature_trg_B)
                    d_out_first_front = dis_model_front(feature_trg[0])
                    loss_adv_trg_first = dis_loss(d_out_first, source_label) + dis_loss(d_out_first_front, source_label)
                    loss_adv = TRAIN_LAMBADA_ADV_FIRST * loss_adv_trg_first
                    # scaler.scale(loss_adv).backward()
                    # if ni%look_iter == 0:
                    #     print('cheat loss(T-->S):', float(loss_adv))

                    d_out_first_src = dis_model(feature_src_B)
                    d_out_first_src_front = dis_model_front(feature_src[0])
                    loss_adv_src_first = dis_loss(d_out_first_src, target_label) + dis_loss(d_out_first_src_front, target_label)
                    loss_adv_src = TRAIN_LAMBADA_ADV_FIRST * loss_adv_src_first
            ...
            generate_F_B_model = deal_grad(generate_F_B_model, key=True)
            generate_F_G_model = deal_grad(generate_F_G_model, key=True)
            with amp.autocast(enabled=cuda):
                feature_src_B = generate_F_B_model([feature_src[0].detach(),feature_src[1].detach(),feature_src[2].detach(),feature_src[3].detach()])
                feature_trg_B = generate_F_B_model([feature_trg[0].detach(),feature_trg[1].detach(),feature_trg[2].detach(),feature_trg[3].detach()])
                feature_src_G = generate_F_G_model(feature_src_B)
                feature_trg_G = generate_F_G_model(feature_trg_B)
                loss_cycle_src = nn.L1Loss()(feature_src_G, src_back_cycle)/2
                loss_cycle_trg = nn.L1Loss()(feature_trg_G, trg_back_cycle)/2
                loss_all_src = loss_cycle_src + loss_SRC
                loss_all_trg = loss_cycle_trg + loss_adv
                scaler.scale(loss_all_src).backward()
                scaler.scale(loss_all_trg).backward()
            ...
            dis_model = deal_grad(dis_model, key=True)
            dis_model_front = deal_grad(dis_model_front, key=True)
            generate_F_B_model = deal_grad(generate_F_B_model, key=False)
            generate_F_G_model = deal_grad(generate_F_G_model, key=False)
            with amp.autocast(enabled=cuda):
                feature_src_B = feature_src_B.detach()
                d_out_first = dis_model(feature_src_B)
                ...
                feature_trg_B = feature_trg_B.detach()
                d_out_first = dis_model(feature_trg_B)
                ...
                ################################################
                d_out_first = dis_model_front(feature_src[0].detach())
                loss_d_first_front = dis_loss(d_out_first, source_label)  # 0.13989
                loss_d_first_front = loss_d_first_front / 2
                scaler.scale(loss_d_first_front).backward()
                if ni % look_iter == 0:
                    print('source: loss_d_first_front\t', float(loss_d_first_front))

                d_out_first = dis_model_front(feature_trg[0].detach())
                loss_d_first_front = dis_loss(d_out_first, target_label)  # 0.94167
                loss_d_first_front = loss_d_first_front / 2
                scaler.scale(loss_d_first_front).backward()
                if ni % look_iter == 0:
                    print('target: loss_d_first_front\t', float(loss_d_first_front))
            ...
            if ni % accumulate == 0:
                ...
                scaler.step(optimizer_d_first)
                scaler.step(optimizer_cycle_B)
                scaler.step(optimizer_cycle_G)
                scaler.step(optimizer_d_first_front)
                scaler.update()
                optimizer.zero_grad()
                optimizer_d_first_front.zero_grad()
                optimizer_d_first.zero_grad()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='./runs/train_forpaper/exp28/weights/best_mr.pt', help='initial weights path')#哪种yolo结构？
    ...
    parser.add_argument('--data', type=str, default='data/city_to_foggy.yaml', help='data.yaml path')#数据集目录#city_to_foggy#citytocaltech
    ...
    parser.add_argument('--batch-size', type=int, default=4, help='total batch size for all GPUs')#batchsize
    ...
    parser.add_argument('--project', default='runs/trainforpaper_ctf_lowfc_hr_twodis', help='save to project/name')
