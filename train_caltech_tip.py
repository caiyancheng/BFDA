...
def train(hyp, opt, device, tb_writer=None,MR=True,if_SAM=False):
    ...
    if if_SAM:
        optimizer = SAM(pg0,optim.SGD,rho=0.5,adaptive=True,lr = hyp['lr0'], momentum=hyp['momentum'], nesterov=True)

    # 设置weight、bn权重的优化方式
    if if_SAM:
        optimizer.add_param_group({'params': pg1,'dampening':0, 'weight_decay': hyp['weight_decay']})  # add pg1 with weight_decay
    else:
        optimizer.add_param_group({'params': pg1, 'weight_decay': hyp['weight_decay']})
    #设置bias的优化方式
    if if_SAM:
        optimizer.add_param_group({'params': pg2, 'dampening':0, 'weight_decay': 0})  # add pg2 (biases)
    else:
        optimizer.add_param_group({'params': pg2})
    print('optimizer_now:', optimizer)
    print('if_SAM',if_SAM)
    ...
    if pretrained:
        ...
        start_epoch, best_fitness = 0, 0.0#cyc改
        if opt.resume:
...
# ###########################################################################################################################
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        optimizer.zero_grad()
        for i, (imgs, targets, paths, _) in pbar:#0,24,220  # batch -------------------------------------------------------------
            ...
            if ni <= nw:#1000
                ...
                accumulate = max(1, np.interp(ni, xi, [1, nbs / total_batch_size]).round())#1
            ...
            with amp.autocast(enabled=cuda):
                _,pred,_,_,_ = model(imgs)  # forward
            ...
            if ni % accumulate == 0:
                ...
                if if_SAM:
                    ...
                    with amp.autocast(enabled=cuda):
                        _,pred2, _, _, _ = model(imgs)  # forward
                        loss2, loss2_items = compute_loss(pred2, targets.to(device))  # loss scaled by batch_size
                        if rank != -1:
                            loss2 *= opt.world_size  # gradient averaged between devices in DDP mode
                        if opt.quad:
                            loss2 *= 4.
                    loss2.backward()
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                ...
                if MR:
                    results, maps, times, MRs = test.test(data_dict,
                                                     ...
                                                     Datasets=opt.data.split('.')[0].split('/')[1],
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser()#./runs/train_forpaper/exp17/weights/best_mr.pt#./runs/train_forpaper/exp28/weights/best_mr.pt
    parser.add_argument('--weights', type=str, default='yolov5l6.pt', help='initial weights path')#哪种yolo结构？
    parser.add_argument('--cfg', type=str, default='', help='model.yaml path')#参考.yaml文件
    parser.add_argument('--data', type=str, default='data/caltech.yaml', help='data.yaml path')#数据集目录
    ...
    parser.add_argument('--device', default='1,2,3,0', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    ...
    parser.add_argument('--project', default='runs/train_tip_caltech_640_640_mosaic', help='save to project/name')
