...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        for j in range(depth):
            if embed_dim == 64 or embed_dim == 192:
                blocks.append(
                    Block(
                        dim_in=int(embed_dim / 2),
                        dim_out=int(embed_dim / 2),
                        num_heads=num_heads,
                        mlp_ratio=mlp_ratio,
                        qkv_bias=qkv_bias,
                        drop=drop_rate,
        ...
        self.HR1 = HRNet(16, 32, 0.1, 64)
        self.HR2 = HRNet(16, 96, 0.1, 192)
        self.down_c_1 = nn.Conv2d(64,32, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
        self.down_c_2 = nn.Conv2d(192,96, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
        # self.down_c_3 = nn.Conv2d(384, 192, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
        # self.HR3 = HRNet(16, 192, 0.1, 384)
    ...
    def forward(self, x):
        ...
        if m == 0:
            hr_output = self.HR1(x)
            x = self.down_c_1(x)
        if m == 1:
            hr_output = self.HR2(x)
            x = self.down_c_2(x)
        # if i == 2:
        #     hr_output = self.HR3(x)
        #     x = self.down_c_3(x)
        B, C, H, W = x.size()
...
class ConvolutionalVisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages): #3
            x, cls_tokens = getattr(self, f'stage{i}')([x,i])
        if self.cls_token:
...
class Base_cvt_cifar(nn.Module):
    def __init__(self, pth, stage_t=[1,2,10]):
        super().__init__()
...
if __name__ == '__main__':
    ...
    md = Base_cvt_cifar(pth,stage_t=[1,1,1])
    ops, params = get_model_complexity_info(md, (3, 224, 224), as_strings=True,
