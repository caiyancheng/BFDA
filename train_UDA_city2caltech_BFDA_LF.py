...
from cycle_generator import get_F_G, get_F_G_only
...
model_trans = 'cvt_simple_18' #cvt#vit#cvt_official
IF_SC_dis = False
...
###########################################################下面就是按照iter计数
        ...
        for i, (src,dis_src,dis_trg) in pbar:#源域和目标域
            ...
            # feature_src[0] = F.interpolate(feature_src[0], size=(224, 224), mode='bilinear', align_corners=True)
            # feature_trg[0] = F.interpolate(feature_trg[0], size=(224, 224), mode='bilinear', align_corners=True)
            loss_adv = 0
            ...
            with amp.autocast(enabled=cuda):
                ...
                feature_src_G = FGM_model(feature_src[0])
                feature_trg_G = FGM_model(feature_trg[0])
                feature_src_G = F.interpolate(feature_src_G, size=(256, 256), mode='bilinear')
                feature_trg_G = F.interpolate(feature_trg_G, size=(256, 256), mode='bilinear')
            with amp.autocast(enabled=cuda):
                loss_cycle_src = nn.L1Loss()(feature_src_G, src_back_cycle)
                loss_cycle_trg = nn.L1Loss()(feature_trg_G, trg_back_cycle)
                loss_trg_all = loss_adv + loss_cycle_trg * Train_loss_prop_gen
                loss_src_all = loss_cycle_src * Train_loss_prop_gen + loss_SRC
                scaler.scale(loss_trg_all).backward()
                scaler.scale(loss_src_all).backward()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--project', default='runs/UDA_city2caltech_BFDA_LF', help='save to project/name')
