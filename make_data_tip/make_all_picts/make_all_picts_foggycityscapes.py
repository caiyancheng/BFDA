...
cityscapes_path_dict = {
    'original_images_path': r'F:\datasets\foggycityscape\leftImg8bit_trainvaltest_foggy\leftImg8bit_foggy',
}

aim_image_path = r'F:\BFDA_datasets\foggycityscapes\images'
    ...
    for subdir_n in tqdm(range(len(image_dir_list))):
        ...
        for image_name in image_subdir_list:
            ...
            for beta in beta_list:
                if str(beta) in image_name:
                    aim_img_dir = os.path.join(aim_image_path, f"{set}_{str(beta).split('.')[-1]}", image_dir_list[subdir_n])
