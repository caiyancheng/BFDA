...
Train_loss_prop_gen = 0.1
Train_loss_prop_dis = 0.01
...
Weight_Range = [1., 1.5, 2.0, 2.5, 3.0, 5.0]
# Weight_Range = [1., 1.5, 2.0, 3.0, 5.0, 10.0]
Weight_Add = 0.2
Foreground_Value = 1
...
def train(hyp, opt, device, tb_writer=None, MR=True):
    ...
    dis_src_train_loader, _ = create_dataloader(train_path, imgsz, batch_size, gs, opt,
                                            ...
                                            world_size=opt.world_size, workers=opt.workers,
                                            image_weights=opt.image_weights, quad=opt.quad, prefix=colorstr('source_train: '))
...
###########################################################下面就是按照iter计数
        ...
        for i, (src,dis_src,dis_trg) in pbar:#源域和目标域
            ...
            src_shortrange_weight_map = make_source_shortrange_weight_map(imgs=imgs_src.clone(), real_labels=real_labels_src,
                                                                          weight_range=Weight_Range, weight_add=Weight_Add,
                                                                          foreground_value=Foreground_Value)
            src_shortrange_weight_map = F.interpolate(src_shortrange_weight_map[:,None,...], size=(256, 256), mode='bilinear')
            src_shortrange_weight_map = src_shortrange_weight_map.to(device)

            if SHOW and epoch == 0 and src_show_i>=0:
                src_img_show = unloader(src_back_cycle[src_show_i].squeeze(0))
                cv2.imwrite(str(save_dir) + '/src_img_aachen_000031_000019_src_cycle_label.png',
                            np.array(src_img_show)[..., ::-1])
                src_shortrange_weight_map_show = unloader(src_shortrange_weight_map[src_show_i].squeeze(0)  / 2)
                cv2.imwrite(str(save_dir) + '/src_img_aachen_000031_000019_src_shortrange_weight_map.png',
                            np.array(src_shortrange_weight_map_show))
            ...
            with amp.autocast(enabled=cuda):
                ...
                trg_shortrange_weight_map = make_target_shortrange_weight_map(imgs=imgs_trg_new_back.clone(), out_nms=out_trg_NMS,
                                                                              weight_range=Weight_Range, weight_add=Weight_Add,
                                                                              foreground_value=Foreground_Value)
                trg_shortrange_weight_map = F.interpolate(trg_shortrange_weight_map[:,None,...], size=(256, 256), mode='bilinear')
                trg_shortrange_weight_map = trg_shortrange_weight_map.to(device, non_blocking=True)
            ...
            if SHOW and epoch == 0 and trg_show_i >= 0:
                ...
                trg_shortrange_weight_map_show = unloader(trg_shortrange_weight_map[src_show_i].squeeze(0) / 2)
                cv2.imwrite(str(save_dir) + '/trg_img_set00_V000_242_background_trg_shortrange_weight_map.png',
                            np.array(trg_shortrange_weight_map_show))
            ...
            if Cheat: #让目标域的图片被认为来自源域
                with amp.autocast(enabled=cuda):
                    d_out_first = LSD_model(feature_trg_B * trg_shortrange_weight_map)
                    ...
                    d_out_first_src = LSD_model(feature_src_B * src_shortrange_weight_map)
            ...
            with amp.autocast(enabled=cuda):
                ...
                d_out_first = LSD_model(feature_src_B * src_shortrange_weight_map)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/citytocaltech.yaml', help='data.yaml path')#数据集目录#city_to_foggy#citytocaltech
    ...
    parser.add_argument('--img-size', nargs='+', type=int, default=[2048, 640], help='[train, test] image sizes')#切割
    ...
    parser.add_argument('--project', default='runs/UDA_city2caltech_BFDA_full_shortrange_improve', help='save to project/name')
