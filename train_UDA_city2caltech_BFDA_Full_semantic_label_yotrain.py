...
# Paste_size = (512,1024)
Paste_size = (1024,2048)

def train(hyp, opt, device, tb_writer=None, MR=True):
    print('Loading the semantic model')
    ss_model = torchvision.models.segmentation.deeplabv3_resnet101(pretrained=True).to(device).eval()
...
###########################################################下面就是按照iter计数
        ...
        for i, (src,dis_src,dis_trg) in pbar:#源域和目标域
            ...
            src_back_cycle = paste_label_semantic(imgs=imgs_src.clone(), ss_model=ss_model, size=Paste_size)
            ...
            with amp.autocast(enabled=cuda):
                ...
                trg_back_cycle = paste_label_semantic(imgs=imgs_trg_new_back.clone(), ss_model=ss_model, size=Paste_size)
            ...
            if Cheat: #让目标域的图片被认为来自源域
                with amp.autocast(enabled=cuda):
                    d_out_first = LSD_model(feature_trg_B)
                    ...
                    d_out_first_src = LSD_model(feature_src_B)
            ...
            with amp.autocast(enabled=cuda):
                ...
                scaler.scale(loss_cycle * Train_loss_prop_gen + loss_adv + loss_SRC).backward()
            ...
            with amp.autocast(enabled=cuda):
                ...
                d_out_first = LSD_model(feature_src_B)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--project', default='runs/UDA_city2caltech_BFDA_full_semantic_label_yotrain', help='save to project/name')
