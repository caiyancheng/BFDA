...
class HRNet(nn.Module):
    def __init__(self, c=48, nof_joints=17, bn_momentum=0.1,in_c = 64):
        super(HRNet, self).__init__()

        # Input (stem net)
        # self.conv1 = nn.Conv2d(3, 64, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)
        # self.bn1 = nn.BatchNorm2d(64, eps=1e-05, momentum=bn_momentum, affine=True, track_running_stats=True)
        self.conv2 = nn.Conv2d(in_c, 256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
        self.bn2 = nn.BatchNorm2d(256, eps=1e-05, momentum=bn_momentum, affine=True, track_running_stats=True)
        self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self, x):  #[4,3,224,224]
        ...
        # x = self.bn1(x)
        # x = self.relu(x)
        x = self.conv2(x)  #[4,64,56,56]
        x = self.bn2(x)
        x = self.relu(x)

        # x = self.layer1(x)  #一堆残差边
...
if __name__ == '__main__':
    ...
    model = HRNet(16, 64, 0.1)
    ops, params = get_model_complexity_info(model, (64, 64, 64), as_strings=True,
    ...
    y = model(torch.ones(4, 64, 64, 64).to(device))
    print(y.shape)
