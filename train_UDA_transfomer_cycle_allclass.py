...
import test_allclass  # import test.py to get mAP after each epoch
...
# from feature_distill import F_D
...
def train(hyp, opt, device, tb_writer=None, MR=True):
    ...
    generate_F_B_model = F_D(input_channel=39)
...
###########################################################################################################################
    ...
    optimizer_d_first = optim.Adam(dis_model.parameters(), lr=TRAIN_LEARNING_RATE_D_ddcpp,
                                 betas=(0.9, 0.99))
...
###########################################################下面就是按照iter计数
        ...
        for i, (src,dis_src,dis_trg) in pbar:#源域和目标域
            ...
            if pattern == 'train':
                src_show_i = find_pict(pict_name='data/all_class/cityscapes/images/train/aachen/aachen_000031_000019_leftImg8bit.png',
                                       paths=paths)
                trg_show_i = find_pict(pict_name='data/all_class/foggycityscapes/images/train/aachen/aachen_000031_000019_leftImg8bit_foggy_beta_0.02.png',  # _small
                                       paths=paths_trg)
            if pattern == 'debug':
                src_show_i = find_pict(pict_name='data/all_class/cityscapes/images/train_debug/aachen/aachen_000031_000019_leftImg8bit.png',
                                       paths=paths)#_full
                trg_show_i = find_pict(pict_name='data/all_class/foggycityscapes/images/train_debug/aachen/aachen_000031_000019_leftImg8bit_foggy_beta_0.02.png',#_small
                                       paths=paths_trg)
            ...
            if SHOW and epoch == 0 and src_show_i>=0:
                ...
                cv2.imwrite(str(save_dir) + '/src_img_aachen_000031_000019_src_cycle_label.png', np.array(src_img_show))#黑白图片就不能再转这个了
            ...
            with amp.autocast(enabled=cuda):
                ...
                print("out_trg: ",out_trg[0][0])
                
                
                out_trg_NMS = non_max_suppression(out_trg, conf_thres=0.3, iou_thres=0.6, labels=[], multi_label=True)
                
                print("out_trg_NMS: ",out_trg_NMS)
            ...
            with amp.autocast(enabled=cuda):
                feature_src_B = generate_F_B_model(feature_src[0])
                feature_trg_B = generate_F_B_model(feature_trg[0])
            ...
            with amp.autocast(enabled=cuda):
                feature_src_B = generate_F_B_model(feature_src[0].detach())
                feature_trg_B = generate_F_B_model(feature_trg[0].detach())
            ...
            with amp.autocast(enabled=cuda):
                ...
                scaler.scale(loss_cycle_src).backward()
                scaler.scale(loss_cycle_trg).backward()
            ...
            if opt.iter_test and epoch > 0:
                if ni % 50 == 0:
                    results, maps, times = test_allclass.test(data_dict,
                                                          batch_size=8,
                                                          imgsz=imgsz_test,
                                                          model=ema.ema,
                                                          ...
                                                          # Datasets='foggycity',
                                                          is_coco=is_coco)
                    print('epoch:  '+str(epoch)+'\t'+'iter:  '+str(i)+ '\t'+ 'Map@0.5:  '+ str(results[2]*100))
    ...
    if rank in [-1, 0]:
        ...
        if opt.data.endswith('coco.yaml') and nc == 80:  # if COCO
            for m in (last, best) if best.exists() else (last):  # speed, mAP tests
                results, _, _ = test_allclass.test(opt.data,
                                              batch_size=batch_size_test,
                                              imgsz=imgsz_test,
                                              conf_thres=0.001,
                                              iou_thres=0.7,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='./runs/best.pt', help='initial weights path')#哪种yolo结构？
    ...
    parser.add_argument('--cfg', type=str, default="yolov5x.yaml", help='model.yaml path')#参考.yaml文件
    parser.add_argument('--data', type=str, default='data/city_to_foggy_allclass.yaml', help='data.yaml path')#数据集目录#city_to_foggy#citytocaltech
    ...
    parser.add_argument('--img-size', nargs='+', type=int, default=[1024, 1024], help='[train, test] image sizes')#切割
    ...
    parser.add_argument('--project', default='runs/trainforpaper_cycle_allclass', help='save to project/name')
