...
class COCOeval: 
    ...
    def evaluate(self, id_setup):
        ...
        if p.useCats:
            p.catIds = list(np.unique(p.catIds))#[0,1,2,3,4,5]
        p.maxDets = sorted(p.maxDets)#[1000]
        self.params=p
        ...
        maxDet = p.maxDets[-1]
        HtRng = self.params.HtRng[id_setup]#[[50, 10000000000.0], [50, 10000000000.0], [50, 10000000000.0], [50, 10000000000.0]]
        VisRng = self.params.VisRng[id_setup]#[[0.65, 10000000000.0], [0.9, 10000000000.0], [0.65, 0.9], [0, 0.65]]
    ...
    def evaluateImg(self, imgId, catId, hRng, vRng, maxDet):
        ...
        dtind = [i for i in range(len(dt)) if
                 dt[i]['bbox'][-1] >= hRng[0] / self.params.expFilter and dt[i]['bbox'][-1] < hRng[
                     1] * self.params.expFilter]
        dt = [d for d in dt if
              d['bbox'][-1] >= hRng[0] / self.params.expFilter and d['bbox'][-1] < hRng[1] * self.params.expFilter]
